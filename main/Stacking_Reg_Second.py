...
def scale_t_columns(df):
    """
    Scale the t-columns (lagged features) using RobustScaler.

    Args:
    df (pd.DataFrame): Input DataFrame

    Returns:
    tuple: (scaled DataFrame, fitted RobustScaler)
    """
    # Identify t-columns
...
def randomized_hyper_tuning(model, X, y, param_dist):
    ...
    param_dist (dict): Parameter distribution for randomized search

    Returns:
    object: Best estimator found by randomized search
    """
...
def get_rand_param_dist_for_regressor(regressor: str):
    """
    Get the parameter distribution for a given regressor.

    Args:
    regressor (str): Name of the regressor

    Returns:
    dict: Parameter distribution for the specified regressor
    """
    param_grid = {
        "xgb": {
        ...
        "theil_sen": {
            'max_subpopulation': randint(5000, 10000),
            'n_subsamples': randint(50, 500),
...
def get_random_stacking_regressor(list_of_regressors):
    ...
    Args:
    list_of_regressors (list): List of tuples (regressor_name, regressor_object)

    Returns:
    StackingRegressor: A StackingRegressor object with randomly selected base models
    """
